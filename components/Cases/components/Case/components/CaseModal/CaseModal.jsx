...
const removeElementFromDOM = event => {
  const iframe = document.querySelector(`.${styles.documentPreview}`); //no such class
  ...
  // 1) need to use method removeEventListener
  // 2) recursion seems to be infinite
...
export default function CaseModal(props) {
  ...
  // await is used inside , so in should be async function
  function proceedPreview(documentId) {
    // getPreview function is missing
    ...
    // no such function removeElementsFromDom, we have removeElementFromDom
    // we add listener on the whole document, so basically it does not listen for the outside click. It just listens to the click
    // we can check if e.target is not inside element (our document inside modal, etc) and the proceed with next actions only
    ...
    iframe.src = url;
    // do we append iframe to body for we need to show it inside modal as <div dangerouslySetInnerHTM=L ...>
    document.body.appendChild(iframe);
    document.body.appendChild(hideBlock);
    // no styles for such class
      ...
      <Modal show={true} onHide={onModalClose}>
        <div className={`${styles.body} modal-body row`}>
          ...
          <div className={styles.description}>
            ...
            <p>
              {/*t(description has to return translated string , not html to parse)*/}
                  ...
                  <Trans defaults={document.name} i18nKey={document.name}/>
                    ...
                    <div className={styles.preview}>
                      {isLoading[document.documentId] ?
                        <Spinner active className={styles.spinner}/> //no such class spinner
                        :
                        <FontAwesomeIcon
                          icon={faEye}
                          data-for="case"
